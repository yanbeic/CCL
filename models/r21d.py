...
class R2Plus1DNet(nn.Module):
    ...
    def __init__(self, layer_sizes, block_type=SpatioTemporalResBlock, with_classifier=False, return_conv=False,
                 num_classes=101, use_features=False, local_features=False):
...
def generate_model(model_depth=18, **kwargs):
    if model_depth == 34:
        print('r21d34')
        model = R2Plus1DNet(layer_sizes=(3, 4, 6, 3), with_classifier=True, **kwargs)
    else:
        model = R2Plus1DNet(layer_sizes=(1, 1, 1, 1), with_classifier=True, ** kwargs)
    return model
